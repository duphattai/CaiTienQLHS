...
IF( NOT EXISTS (SELECT [dbo].[HOSOHOCSINH].MAHOCSINH, NGAYSINH
				FROM [dbo].[XEPLOP], [dbo].[LOP], [dbo].THAMSO, [dbo].HOSOHOCSINH
				WHERE [dbo].[XEPLOP].MALOP = [dbo].[LOP].MALOP AND [dbo].[LOP].NAMHOC = @NAMHOC AND HOSOHOCSINH.MAHOCSINH = XEPLOP.MAHOCSINH
...
BEGIN
		...
		FROM [dbo].[HOSOHOCSINH], THAMSO
		WHERE (CONVERT(int, SUBSTRING(@NAMHOC, 1, 4)) - year(NGAYSINH)) >=  (SELECT TOP 1 [TUOITOITHIEU] FROM THAMSO)
				AND (CONVERT(int, SUBSTRING(@NAMHOC, 1, 4)) - year(NGAYSINH)) <=  (SELECT TOP 1 [TUOITOIDA] FROM THAMSO)
END
ELSE
BEGIN
